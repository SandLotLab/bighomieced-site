...
# Usage:
#   .\site-audit.ps1
#   .\site-audit.ps1 -Domain "https://bighomieced.app" -AdsClient "ca-pub-XXXXXXXXXXXXXXX"

[CmdletBinding()]
param(
  [Parameter(Mandatory=$false)]
  [string]$Domain = "https://bighomieced.app",

  [Parameter(Mandatory=$false)]
  [string]$AdsClient = ""  # e.g. "ca-pub-7167291111213614" (optional)
)

# Normalize domain (remove trailing slash)
$Domain = $Domain.TrimEnd("/")

# Ensure TLS 1.2+ where available
...
function Fetch([string]$url) {
  try {
    return Invoke-WebRequest -Uri $url -Method GET -MaximumRedirection 10 -ErrorAction Stop -Headers @{
...
function Fetch-Follow308([string]$url, [int]$maxHops = 10) {
  $current = $url
  for ($i = 0; $i -lt $maxHops; $i++) {
    try {
      $r = Invoke-WebRequest -Uri $current -Method GET -MaximumRedirection 0 -ErrorAction Stop -Headers @{
        "User-Agent" = "Mozilla/5.0"
        "Accept"     = "*/*"
      }
      return $r
...
if (-not $homeResp) {
  ...
  if ([string]::IsNullOrWhiteSpace($AdsClient)) {
    $hasLoader = ($homeResp.Content -match "pagead2\.googlesyndication\.com/pagead/js/adsbygoogle\.js")
    if ($hasLoader) {
      Write-Host "AdSense loader detected on homepage."
    } else {
      Write-Host "AdSense loader NOT detected on homepage."
    }
  } else {
    $hasLoader = ($homeResp.Content -match "pagead2\.googlesyndication\.com/pagead/js/adsbygoogle\.js")
...
if ($adsResp) {
  if ([string]::IsNullOrWhiteSpace($AdsClient)) {
    Write-Host "ads.txt fetched. (Publisher ID check skipped: AdsClient not set.)"
  } else {
...
function Print-Hrefs([string]$label, [string]$html) {
  Write-Host "`n--- $label ---"
...
try {
  ...
  try { Invoke-WebRequest "$Domain/robots.txt"  -OutFile (Join-Path $backupPath "audit_robots.txt")  -ErrorAction Stop } catch {}
  try { Invoke-WebRequest "$Domain/sitemap.xml" -OutFile (Join-Path $backupPath "audit_sitemap.xml") -ErrorAction Stop } catch {}
  try { Invoke-WebRequest "$Domain/ads.txt"     -OutFile (Join-Path $backupPath "audit_ads.txt")     -ErrorAction Stop } catch {}
